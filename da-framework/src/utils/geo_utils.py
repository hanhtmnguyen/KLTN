...
def sample_raster_at_points(
    ...
    with rasterio.open(raster_path) as src:
        # Reproject points temporarily to match raster CRS if they differ
        if points.crs != src.crs:
            logger.debug("Reprojecting points from %s to %s for sampling %s", points.crs, src.crs, Path(raster_path).name)
            sample_points = points.to_crs(src.crs)
        else:
            sample_points = points
            
        coords = [(pt.x, pt.y) for pt in sample_points.geometry]
