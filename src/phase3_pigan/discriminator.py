"""
PI-GAN Discriminator — PatchGAN.

Classifies whether local patches of flood maps are real (from HEC-RAS)
or fake (from the generator).  Focuses on local texture realism, which
is important for capturing sharp flood boundaries around buildings.
"""
...
class PatchGANDiscriminator(nn.Module):
    ...
    Outputs a 2-D map of patch-level real/fake probabilities rather
    than a single scalar, encouraging sharper local details.
    ...
    def __init__(
        ...
        # Output layer: 1-channel patch map
